...
const styles = html`
  <style>
    ...
    .preview {
      background: #32a0f4; /* fallback for old browsers */
...
class VacuumCard extends LitElement {
  ...
  renderSource() {
    ...
    >
      <paper-button class="source-menu__button" slot="dropdown-trigger">
        <span class="source-menu__source" show=${true}> ${source} </span>
  ...
  renderVacuumStats() {
    const {
      attributes: { mainBrush, sideBrush, filter, sensor },
    } = this.entity;
  ...
  renderCleaningStats() {
    const {
      attributes: { currentCleanArea, currentCleanTime },
    } = this.entity;

    return html`
      <div class="stats-block">
        <span class="stats-hours">${currentCleanArea}</span> m<sup>2</sup>
        <div class="stats-subtitle">Cleaning area</div>
      </div>
      <div class="stats-block">
        <span class="stats-hours">${currentCleanTime}</span> minutes
  ...
  render() {
    const {
      state,
      attributes: { battery_level, battery_icon },
    } = this.entity;
